...
func TestGenericScheduler(t *testing.T) {
	...
	for _, test := range tests {
		...
		scheduler := NewGenericScheduler(
			cache, nil, NewSchedulingQueue(nil), test.predicates, algorithm.EmptyPredicateMetadataProducer, test.prioritizers, algorithm.EmptyMetadataProducer, []algorithm.SchedulerExtender{}, nil)
...
func TestPreempt(t *testing.T) {
	...
	for _, test := range tests {
		...
		scheduler := NewGenericScheduler(
			cache, nil, NewSchedulingQueue(nil), map[string]algorithm.FitPredicate{"matches": algorithmpredicates.PodFitsResources}, algorithm.EmptyPredicateMetadataProducer, []algorithm.PriorityConfig{{Function: numericPriority, Weight: 1}}, algorithm.EmptyMetadataProducer, extenders, nil)
